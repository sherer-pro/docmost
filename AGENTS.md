...
- Production Typesense is defined by the checked-in `compose.typesense.yml`; include it with `compose.production.yml` in every production lifecycle command when `SEARCH_DRIVER=typesense`. Keep `8108` private and preserve its exact digest, limits, healthcheck, dropped capabilities, and log rotation.
...
- RAG content controls: production Compose forwards `RAG_SYNC_METADATA_WRITE_VERSION` (default `2`) and `RAG_CONTENT_PROCESSORS_ENABLED` (default `attachment-text-v1`) to API and collaboration services. Keep these defaults when upgrading; new retrieval modes remain opt-in per space.
