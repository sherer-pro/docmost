...
- единую проекцию знаний `projectionVersion: 2` для включённых полей документов и custom fields, именованных свойств и ячеек баз данных, строк баз данных и активных источников `dictionary_term`;
- необязательный поиск Open WebUI `hybrid_with_vector_fallback` и контекстное уточнение поискового запроса через `followUpRewriteEnabled` с безопасным возвратом к исходному запросу;
- возможности scope `schemaVersion: 4`, описывающие включённые обработчики контента и поддерживаемые типы вложений;
...
- политики шаблонов на уровнях развёртывания, пространства и группы с явным сохранением и проверкой версии; владельцы и администраторы рабочей области, а также администраторы пространства обходят ограничения групп, но подчиняются выключателям сервера и пространства; настройки пространств открываются на отдельных страницах разделов, а административный список поддерживает поиск, архивные фильтры и безопасную сводку функций (см. [управление пространствами](docs/SPACE_ADMINISTRATION.md));
