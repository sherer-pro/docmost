...
Маршрут `/settings/ai/guide` содержит многоуровневое руководство по AI, RAG и
MCP. Устойчивые hash-ссылки ведут к разделам помощника, query-time retrieval,
внешнего RAG API, встроенного Open WebUI RAG Sync, входящего `/mcp`, исходящих
MCP-серверов, безопасности и диагностики. Карточки задач показывают
ответственного, prerequisite, ожидаемый результат и прямой переход в нужные
настройки; раскрываемые инструкции добавляют последовательность
Save → Test → Enable, признаки успеха и безопасный rollback.

В справке есть матрица учётных данных и границ безопасности, copy-кнопки для
маршрутов и переменных окружения, разбор `401`, `409`, `429`, `503`, потери
Redis lease, `source_access_changed`, `cleanup required` и отозванного MCP
consent. Четыре локализованные Mermaid-схемы рендерятся в strict-режиме,
проходят SVG sanitizer, открываются на весь экран с клавиатуры и всегда имеют
текстовую пошаговую альтернативу.

Руководство явно разделяет контуры: query-time retrieval выполняет поиск во
время ответа; `/api/rag/*` служит только для внешнего read-only экспорта и после
повторной авторизации может отдавать защищённые attachment streams; встроенный
RAG Sync пишет в Open WebUI через внутренние сервисы и не вызывает публичный
RAG API. Входящий `/mcp` отдаёт metadata вложений, но не их бинарное содержимое.
Контекстные ссылки на справку доступны со страницы API-ключей и, для
workspace-admin, из настроек пространства.
