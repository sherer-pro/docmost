...
At the application level, `apps/server/src/app.module.ts` wires the HTTP API, core domain module, general Socket.IO relay, queue, static frontend serving, health, import/export, storage, mail, security/version support, telemetry, Redis, and database access. It deliberately does not import the Hocuspocus collaboration runtime. `apps/server/src/collaboration/server/collab-app.module.ts` is the only application composition that owns Hocuspocus, `/collab`, Redis document leases, persistence hooks, and the collaboration history worker.
...
Synced blocks use `transclusionSource` and `transclusionReference` nodes. The editor can wrap an allowed top-level selection as a source or create an empty source from the slash menu; nesting a source inside another synced block is rejected. Authenticated lookup subscriptions coalesce `page-embed:invalidate` events and refresh every active key while retaining the last successful content during the ACL-aware retry. Lookup, references, public-share rendering, and unsync remain fail-closed through the services under `apps/server/src/core/page/transclusion`.
...
Markdown, HTML, and PDF exports materialize synced content under the exporting user's current ACL and contain no live service identifiers. Canonical Docmost archive schema v4 preserves and remaps internal synced references, stores access-checked snapshots only for external references, and materializes those external references on import. Legacy `pageEmbed` relationships are never recreated. The importer remains compatible with schema v2 and v3.
