...
The final pre-integration branch image was built from `fdd342c3`; only its local ID prefix `sha256:1a77300` was retained in the command evidence, so it is not presented as a complete digest. After integration, the deployed image was rebuilt from `e7b44a5e` with exact image ID `sha256:13a0735a6322bd384a52843a7f3742aca1e23a06c1a5bdb09dd9ef394a862129`; the container became healthy before the final browser check.
...
The five production commits were merged conflict-free into local `main` as `e7b44a5e`. Files under `graphify-out/*` were dirty before the audit and were never staged or committed. The report commit unexpectedly launched the installed asynchronous Graphify post-commit hook despite `--no-verify`. Four pre-commit files were restored byte-for-byte from Graphify's automatic daily backup and verified against the captured SHA-256 values. Graphify does not back up `graph.html`; its pre-commit SHA-256 was captured as `BDB5CC41B70974EBD823262B8A1E0945602FE5E1249160A4235CFF31197B7171`, but the exact bytes could not be recovered from backups, Git reachable/unreachable blobs, other worktrees, editor history, or the recycle bin. It was regenerated from the exact restored graph and labels; the hook-produced replacement and sidecars were preserved at `D:\DevProjects\docmost-qa-G17\artifacts\g17-comments\post-commit-hook-overwrite-20260811T114404`. This generated-artifact recovery limitation is reported explicitly rather than presented as an exact restoration. No push, pull request, tag, or release was created.
