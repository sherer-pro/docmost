...
export function getSelectionRangeInRow(
  ...
  selection: Selection = tr.selection,
  ...
  for (let i = startRowIndex; i >= 0; i--) {
    const cells = getCellsInRow(i, selection);
    if (cells) {
      cells.forEach((cell) => {
  ...
  for (let i = startIndex; i <= endIndex; i++) {
    const maybeCells = getCellsInRow(i, selection);
  ...
  const firstSelectedRowCells = getCellsInRow(startIndex, selection);
  ...
  for (let i = endIndex; i >= startIndex; i--) {
    const rowCells = getCellsInRow(i, selection);
