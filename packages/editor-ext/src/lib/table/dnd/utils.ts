...
export function getCellCoveringColumn(
  ...
  for (const cell of cells) {
    const nextIndex = logicalIndex + Math.max(1, cell.colSpan);
    if (columnIndex >= logicalIndex && columnIndex < nextIndex) {
      return cell;
    }
...
function getTargetFirstCellDOM(
  ...
  if (direction === 'row') {
    ...
    return row ? getCellCoveringColumn(row, index) : undefined;
  }
}
