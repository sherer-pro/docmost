...
import {
  DATABASE_QUERY_KEYS,
  PAGE_QUERY_KEYS,
} from "@/features/page/queries/query-keys.ts";
...
const DEFAULT_FILTER: IDatabaseFilterCondition =
  defaultDatabaseTableExportState.filters[0];
...
export function DatabaseTableView({
  ...
  const batchUpdateRowsMutation =
    useBatchUpdateDatabaseRowsMutation(databaseId);
  ...
  const [newPropertyType, setNewPropertyType] =
    useState<DatabasePropertyType>("multiline_text");
  ...
  const [
    optimisticallyDeletedPropertyIds,
  ...
  const [filters, setFilters] = useState<IDatabaseFilterCondition[]>([
    DEFAULT_FILTER,
  ]);
  ...
  const [settingsProperty, setSettingsProperty] =
    useState<IDatabaseProperty | null>(null);
  const [propertyNameDrafts, setPropertyNameDrafts] = useState<
    Record<string, string>
  >({});
  const [renamingRowPageId, setRenamingRowPageId] = useState<string | null>(
    null,
  );
  ...
  const [dragOverPropertyId, setDragOverPropertyId] = useState<string | null>(
    null,
  );
  const draggedPropertyIdRef = useRef<string | null>(null);
  const [movingRow, setMovingRow] = useState<IDatabaseRowWithCells | null>(
    null,
  );
  const [copyingRow, setCopyingRow] = useState<IDatabaseRowWithCells | null>(
    null,
  );
  const [selectedRowPageIds, setSelectedRowPageIds] = useState<
    Record<string, boolean>
  >({});
  const [bulkPropertyId, setBulkPropertyId] = useState<string | null>(null);
  const [bulkValue, setBulkValue] = useState("");
  const [bulkCheckboxValue, setBulkCheckboxValue] = useState<"true" | "false">(
    "true",
  );
  ...
  const findDatabaseNodeInTree = (
    ...
    for (const treeNode of nodes) {
      if (
        treeNode.nodeType === "database" &&
        treeNode.databaseId === databaseId
      ) {
        return treeNode;
      }
  ...
  useEffect(() => {
    ...
    try {
      ...
      const normalizedSortState =
        ...
        (parsedState.sortState.direction === "asc" ||
          parsedState.sortState.direction === "desc")
          ? {
      ...
      setFilters(
        normalizedFilters.length > 0 ? normalizedFilters : fallbackFilters,
      );
  ...
  useEffect(() => {
    ...
    setRows((previousRows) => {
      ...
      const rowsByPageId = new Map(
        previousRows.map((row) => [row.pageId, row]),
      );
  ...
  useEffect(
    () => () => {
      ...
      const pendingRowIds = [
        ...new Set(
          pendingRowDeletions.flatMap(
            ([, pendingDeletion]) => pendingDeletion.rowIds,
          ),
        ),
      ];
      ...
      const pendingPropertyDeletions = [
        ...pendingPropertyDeletionsRef.current.entries(),
      ];
      pendingPropertyDeletionsRef.current.clear();
      const pendingPropertyIds = pendingPropertyDeletions.map(
  ...
  const handleCreateRow = async () => {
    ...
    try {
      ...
      console.error(
        "Failed to synchronize database children after row creation",
        error,
      );
    }
  ...
  const propertyTypeLabels = useMemo<Record<DatabasePropertyType, string>>(
    () =>
      Object.fromEntries(
        Object.entries(DATABASE_PROPERTY_TYPE_LABEL_KEYS).map(
          ([propertyType, i18nKey]) => [propertyType, t(i18nKey)],
        ),
  ...
  const checkboxFilterOptions = useMemo(() => getCheckboxFilterOptions(t), [t]);
  ...
  const hasEffectiveCellValueChange = (
    ...
    const normalizedValue = buildDatabaseCellPayloadValue(
      property,
      sourceValue,
    );
    const shouldDelete = shouldDeleteCellPayload(
      property.type,
      normalizedValue,
    );
    ...
    const currentShouldDelete = shouldDeleteCellPayload(
      property.type,
      currentNormalizedValue,
    );
    const nextPersistedValue = shouldDelete ? null : normalizedValue;
    const currentPersistedValue = currentShouldDelete
      ? null
      : currentNormalizedValue;
  ...
  const activeProperties = useMemo(
    () =>
      properties.filter(
        (property) => !optimisticallyDeletedPropertyIds[property.id],
      ),
  ...
  useEffect(() => {
    ...
    const selectedView = databaseViews.find(
  ...
  }, [activeProperties, areDatabaseViewsLoaded, databaseViews, selectedViewId]);
  ...
  useEffect(() => {
    ...
    setFilters((previousFilters) => {
      const nextFilters = previousFilters.filter(
        (condition) =>
          !condition.propertyId || activePropertyIds.has(condition.propertyId),
      );
  ...
  const movePropertyByVisibleOffset = (propertyId: string, offset: -1 | 1) => {
  ...
  const hasPageReferenceColumn = useMemo(
    () =>
      displayedProperties.some(
        (property) => property.type === "page_reference",
      ),
    [displayedProperties],
  );
  const allPagesQuery = useQuery({
    queryKey: [
      ...PAGE_QUERY_KEYS.rootSidebar(spaceId, ["page", "database"]),
      "all-pages",
    ],
    queryFn: () =>
      getAllSidebarPages({
  ...
  const preparedRows = useMemo(() => visibleRows, [visibleRows]);
  ...
  const startEditing = (
    ...
    setEditingValue(
      extractCurrentDatabaseCellValue(getRawCellValue(row, property.id)),
    );
  };
  ...
  const saveEditing = async (
    ...
    const sourceValue =
      typeof nextValue === "undefined" ? editingValue : nextValue;
    const nextCellChange = hasEffectiveCellValueChange(
      row,
      property,
      sourceValue,
    );
        ...
        cells: [
          {
            ...
            value: nextCellChange.shouldDelete
              ? null
              : nextCellChange.normalizedValue,
  ...
  const applyPastedMatrix = async (
    ...
    for (let rowOffset = 0; rowOffset < matrix.length; rowOffset += 1) {
      ...
      for (
        ...
        const targetProperty =
          displayedProperties[startPropertyIndex + propertyOffset];
        if (!targetProperty) {
          break;
        }
        ...
        const nextChange = hasEffectiveCellValueChange(
          targetRow,
          targetProperty,
          sourceValue,
        );
    ...
    const rowOperations = [...rowsPayloadByPageId.entries()].map(
      ([pageId, cells]) => ({
        pageId,
  ...
  const removeRowsFromTree = (pageIds: string[]) => {
    ...
    const rowNodesById = new Map(
      (databaseNode.children ?? []).map((childNode) => [
        childNode.id,
        childNode,
      ]),
    );
  ...
  const scheduleDeleteRows = (rowPageIds: string[]) => {
    const pendingRowPageIds = new Set(
      [...pendingRowDeletionsRef.current.values()].flatMap(
        (pendingDeletion) => pendingDeletion.rowIds,
      ),
    );
  ...
  const confirmDeleteRows = (rowPageIds: string[]) => {
    ...
    modals.openConfirmModal({
      ...
      children: (
        <Text size="sm">
          {isMultipleRows
            ? t("Delete rows confirm with undo", {
                count: uniqueRowPageIds.length,
              })
  ...
  const scheduleDeleteProperty = (property: IDatabaseProperty) => {
    ...
    const timeoutId = setTimeout(() => {
      const scheduledTimeoutId = pendingPropertyDeletionsRef.current.get(
        property.id,
      );
      ...
      message: (
        <Group gap="xs" wrap="nowrap">
          ...
          <Button
            ...
            size="compact-xs"
            onClick={() => {
              const scheduledTimeoutId =
                pendingPropertyDeletionsRef.current.get(property.id);
              if (!scheduledTimeoutId) {
                return;
              }
  ...
  const confirmDeleteProperty = (property: IDatabaseProperty) => {
    modals.openConfirmModal({
      ...
      children: (
        <Text size="sm">
          {t("Delete property confirm with undo and values", {
            name: property.name,
          })}
        </Text>
      ),
  ...
  const applyBulkUpdate = async () => {
    ...
    const targetProperty = activeProperties.find(
            ...
            <>
              ...
              <Menu shadow="md" width={220}>
                ...
                <Menu.Dropdown>
                  ...
                  {isEditable && activeProperties.length > 0 && (
                    <Menu.Divider />
                  )}
      ...
      {isEditable && selectedRowIds.length > 0 && (
        <Paper withBorder radius="sm" p="sm" mb="md">
          <Group wrap="wrap">
            ...
            />
            {activeProperties.find((property) => property.id === bulkPropertyId)
              ?.type === "checkbox" ? (
              <Select
            ...
            <Button variant="subtle" onClick={() => setSelectedRowPageIds({})}>
              {t("Cancel")}
            </Button>
          </Group>
        </Paper>
                  ...
                  <Table.Th w={52} className={classes.selectColumn}>
                    <Checkbox
                      ...
                      checked={
                        ...
                        selectedRowIds.length === preparedRows.length
                      }
                      indeterminate={
                      ...
                      onChange={(event) =>
                        toggleSelectAllPreparedRows(event.currentTarget.checked)
                      }
                    />
                  </Table.Th>
                )}
                          ...
                          <AccessibleActionIcon
                            ...
                            onPointerMove={(event) => {
                              ...
                              setDragOverPropertyId(
                                targetPropertyId &&
                                  targetPropertyId !== movedPropertyId
                                  ? targetPropertyId
                                  : null,
                              );
                            }}
                        ...
                        >
                          ...
                          <Menu.Dropdown>
                            ...
                            <Menu.Item
                              ...
                              onClick={() =>
                                navigate(
                                  `/s/${spaceSlug}/p/${row.page?.slugId ?? row.pageId}`,
                                  {
                      ...
                      return (
                        <Table.Td
                          ...
                          onKeyDown={(event) => {
                            ...
                            void (async () => {
                              ...
                              const nextProperty =
                                displayedProperties[nextCell.propertyIndex];
                        ...
                        >
                          <DatabaseCellRenderer
                            ...
                            onSave={(nextValue) =>
                              saveEditing(row, property, nextValue)
                            }
                          />
                        </Table.Td>
                      );
