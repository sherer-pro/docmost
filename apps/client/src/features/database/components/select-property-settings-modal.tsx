 ...
 *
 * In auto mode, value is always generated by buildAutoOptionValue.
 * For existing options with stored value, keep that value unchanged
 * to preserve backward compatibility during edits.
 */
  ...
  useEffect(() => {
    ...
    const normalizedOptions: SelectOptionDraft[] = initialSettings.options.length
      ? initialSettings.options.map((option) => ({
          ...option,
          // For legacy options without explicit value, enable auto mode so value
          // is reconstructed from label and recalculated on label edits.
