...
const ssoSchema = z
  .object({
    ...
    ldapUrl: z
      .string()
      .url()
      .regex(/^ldaps?:\/\//i, "Must be an LDAP or LDAPS URL"),
  ...
  .refine(
    (values) =>
      values.ldapUrl.toLowerCase().startsWith("ldaps://") ||
      values.ldapTlsEnabled,
    {
      path: ["ldapTlsEnabled"],
      message: "StartTLS is required when using ldap://",
    },
  );
              ...
              <Accordion.Panel>
                <Stack>
                  ...
                  {(form.values.ldapTlsEnabled ||
                    form.values.ldapUrl.toLowerCase().startsWith("ldaps://")) && (
                    <Textarea
