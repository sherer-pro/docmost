...
export function useWorkspacePublicDataQuery(spaceSlug?: string): UseQueryResult<
  IPublicWorkspace,
  Error
> {
  return useQuery({
    queryKey: ["workspace-public", spaceSlug],
    queryFn: () => getWorkspacePublicData(spaceSlug),
  });
}
...
export function useGetInvitationQuery(
  ...
  return useQuery({
    queryKey: ["invitations", invitationId, token],
    queryFn: () => getInvitationById({ invitationId, token }),
