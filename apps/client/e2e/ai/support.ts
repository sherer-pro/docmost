...
export function csrfHeaders(token: string, origin?: string) {
  const base =
    origin ?? process.env.DOCMOST_BASE_URL ?? "http://localhost:3000";
  return {
    Origin: base,
    Referer: `${base}/`,
    "x-csrf-token": token,
  ...
  evidence: [
    async ({ page, context }, use, testInfo) => {
      ...
      if (!authToken || !csrfToken)
        throw new Error(
          "DOCMOST_AUTH_TOKEN and DOCMOST_CSRF_TOKEN are required at runtime",
        );
      const url = new URL(
      ...
      if (!localeResponse.ok()) {
        throw new Error(
          `Failed to select ${locale} for ${testInfo.project.name}: ${localeResponse.status()}`,
        );
      }
      ...
      page.on("pageerror", (error) =>
        consoleEvents.push({ type: "pageerror", text: redact(error.message) }),
      );
      await use();
      ...
      await fs.mkdir(path.join(auditRoot, "console-errors"), {
        recursive: true,
      });
      await fs.writeFile(
        path.join(auditRoot, "network", `${prefix}.json`),
        `${JSON.stringify(network, null, 2)}\n`,
      );
      await fs.writeFile(
        path.join(auditRoot, "console-errors", `${prefix}.json`),
        `${JSON.stringify(consoleEvents, null, 2)}\n`,
      );
      if (testInfo.status !== testInfo.expectedStatus) {
        await fs.mkdir(path.join(auditRoot, "screenshots"), {
          recursive: true,
        });
        await page
          .screenshot({
            path: path.join(auditRoot, "screenshots", `${prefix}.png`),
            fullPage: true,
          })
          .catch(() => undefined);
      }
