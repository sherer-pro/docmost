...
const auditRoot = path.join(repoRoot, "output/audit/ai-assistant-2026-08-07");
...
const mockImage = "mockserver/mockserver@sha256:fed9b2089e021947f785d1f0bfda3723352bb2c1634ce7b0bcd42dfd1b0fd02f";
const mockPort = Number(process.env.DOCMOST_AI_MOCK_PORT ?? 1080);
const providerBaseUrl = process.env.DOCMOST_AI_PROVIDER_BASE_URL ?? `http://127.0.0.1:${mockPort}/v1`;
...
async function responseJson(response) {
  ...
  if (!response.ok()) throw new Error(`${new URL(response.url()).pathname} failed with ${response.status()}: ${text.slice(0, 500)}`);
...
async function ensureRuntimeAuth() {
  if (process.env.DOCMOST_AUTH_TOKEN?.trim() && process.env.DOCMOST_CSRF_TOKEN?.trim()) return;
  ...
  try {
    const response = await loginContext.post("/api/auth/login", { data: { email, password } });
    if (!response.ok()) throw new Error(`Audit admin login failed with ${response.status()}`);
    ...
    if (!authToken || !csrfToken) throw new Error("Audit admin login did not return the required cookies");
...
async function waitForMock() {
  for (let attempt = 0; attempt < 40; attempt += 1) {
    try {
      const response = await fetch(`http://127.0.0.1:${mockPort}/mockserver/status`);
...
async function runNode(script, env = process.env) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [script], { cwd: clientRoot, env, stdio: "inherit" });
...
async function runPlaywright() {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [cli, "test", "--config=playwright.ai.config.ts"], {
      cwd: clientRoot,
      env: process.env,
      stdio: "inherit",
    });
...
for (const directory of ["console-errors", "network", "playwright-artifacts", "playwright-html", "screenshots", "traces"]) {
...
try {
  ...
  await execFileAsync("docker", ["run", "--rm", "-d", "--name", containerName, "-p", `${mockPort}:1080`, mockImage]);
  mockStarted = true;
  await waitForMock();
  const expectations = JSON.parse(await fs.readFile(path.join(import.meta.dirname, "mockserver-expectations.json"), "utf8"));
  const initialized = await fetch(`http://127.0.0.1:${mockPort}/mockserver/expectation`, {
    method: "PUT",
    headers: { "content-type": "application/json" },
    body: JSON.stringify(expectations),
  });
  if (!initialized.ok) throw new Error(`MockServer expectation initialization failed with ${initialized.status}`);

  api = await createApi();
  const space = await responseJson(await api.post("/api/spaces", { data: { name: `AI audit ${runId}`, slug: `aiaudit${runId}`, description: "Isolated deterministic AI assistant audit space." } }));
  spaceId = space.id;
  const page = await responseJson(await api.post("/api/pages", { data: { spaceId, title: "AI audit document", content: { type: "doc", content: [{ type: "paragraph", content: [{ type: "text", text: "AI audit context" }] }] }, format: "json" } }));
  await responseJson(await api.patch(`/api/spaces/${spaceId}/ai/config`, { data: {
    enabled: true,
    agentEnabled: false,
    ...
    visionEnabled: true,
    quickCommands: [{ id: "audit-summary", label: "Audit summary", prompt: "Summarize this document", enabled: true, position: 0 }],
  } }));
  await fs.writeFile(statePath, `${JSON.stringify({ runId, spaceId, spaceSlug: space.slug, pageId: page.id, pageSlugId: page.slugId, pageTitle: page.title, retained: true, mock: { image: mockImage, tagCommit: "6fb02a58ba9f7c6648553aaf85625ff0344f1e53" } }, null, 2)}\n`);

  exitCode = await runPlaywright();
  const scannerEnv = { ...process.env, DOCMOST_AUDIT_CANARY: canary };
  scannerEnv.DOCMOST_AI_AUDIT_ROOT = auditRoot;
  const sanitizerCode = await runNode(path.join(import.meta.dirname, "sanitize-traces.mjs"), scannerEnv);
  if (sanitizerCode !== 0) exitCode = sanitizerCode;
  const scannerCode = await runNode(path.join(import.meta.dirname, "scan-artifacts.mjs"), scannerEnv);
  ...
  const count = (value) =>
    value && appLogText.includes(value) ? appLogText.split(value).length - 1 : 0;
  await fs.writeFile(
  ...
  if (mockStarted) {
    const logs = await execFileAsync("docker", ["logs", containerName], { maxBuffer: 10 * 1024 * 1024 }).catch(() => ({ stdout: "", stderr: "" }));
